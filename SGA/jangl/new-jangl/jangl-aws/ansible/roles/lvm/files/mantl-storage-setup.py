...
def process_fs(sec, params):
    ...
    if fstype == "swap":
        print "--> Create swap for {}".format(dev)
        mountname = dev.lstrip("/").replace("/", "-")
        unit = "{}.swap".format(mountname)
...
def process_thin(sec, params):
    ...
    with closing(open(conf, "w")) as f:
        f.write("MANTL_STORAGE_OPTIONS=--storage-driver devicemapper"
                " --storage-opt dm.thinpooldev=/dev/mapper/{mapper}"
                " --storage-opt dm.use_deferred_removal=true"
                " --storage-opt dm.use_deferred_deletion=true"
                " {extra}".format(mapper=mapper_device, extra=extra))
    check_call(["systemctl", "daemon-reload"])
