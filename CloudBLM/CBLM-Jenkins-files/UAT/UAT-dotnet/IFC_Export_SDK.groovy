...
stage('publish')
{
  //publish and delete the working dir in jenkins
   bat 'dotnet publish CloudBLM-IFCExport.sln -c Release -o D:\\CloudBLM\\IFCSDKService'
   deleteDir()
}
...
checkout([$class: 'SubversionSCM', additionalCredentials: [], excludedCommitMessages: '', excludedRegions: '', excludedRevprop: '', excludedUsers: '', filterChangelog: false, ignoreDirPropChanges: false, includedRegions: '', locations: [[cancelProcessOnExternalsFail: true, credentialsId: '585fa701-720a-4602-abe6-b977c8773a4c', depthOption: 'infinity', ignoreExternalsOption: true, local: '.', remote: 'https://192.168.1.200/svn/CloudBLM-DevOps/deployment/windows-playbook/UAT-Ansible/IFCExportSDK']], quietOperation: true, workspaceUpdater: [$class: 'UpdateUpdater']])      
      ...
      stage('Start IIS website')
      {
        ...
        sh 'ansible-playbook IFCExportSDK-start.yml'
         deleteDir()
      }
  }
}
